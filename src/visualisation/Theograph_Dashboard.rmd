...
ui <- fluidPage(
  ...
  selectizeInput(
    ...
    options = list(placeholder = "All diagnoses")
  ),
  selectizeInput(
    "lab_filter",
    "Biomedical tests / assays",
    choices = NULL, multiple = TRUE,
    options = list(placeholder = "All tests")
  ),
  selectizeInput(
    "drug_filter",
    "Drugs",
...
server <- function(input, output, session) {
  ...
  # Populate the three multi-selects when a patient is chosen
observeEvent(input$patient_names, {
  d <- Pt_Wide_reactive()

  dx_choices <- sort(unique(c(d$Diagnosis_1, d$Diagnosis_2)))
  dx_choices <- dx_choices[!is.na(dx_choices) & dx_choices != ""]
  updateSelectizeInput(session, "dx_filter", choices = dx_choices, server = TRUE)

  lab_choices <- sort(unique(c(d$Biomedical_Test_1, d$Biomedical_Test_2)))
  lab_choices <- lab_choices[!is.na(lab_choices) & lab_choices != ""]
  updateSelectizeInput(session, "lab_filter", choices = lab_choices, server = TRUE)

  drug_choices <- sort(unique(c(d$Drug_Name_1, d$Drug_Name_2)))
  drug_choices <- drug_choices[!is.na(drug_choices) & drug_choices != ""]
  updateSelectizeInput(session, "drug_filter", choices = drug_choices, server = TRUE)
})
