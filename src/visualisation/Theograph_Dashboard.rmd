...
/* Full-width layout: remove R Markdown max-width constraints */
body .main-container,
body .container,
...
/* Accessibility: ensure active tab has sufficient contrast */
...
/* Optional: clearer hover state on inactive tabs */
...
.note-card.clickable:focus{
  outline:3px solid rgba(64,126,201,0.35);
  outline-offset:2px;
}
...
create_time_series_plot <- function(data, date_col, y_col, test_col, unit_col, vis_colour) {
  tryCatch({
    ...
    get_reference_line <- function(test_name) {
      tn <- as.character(test_name)
      if (is.na(tn) || tn == "") return(NULL)

      if (tn == "Serum LDL (non-HDL)") {
        return(list(y = 3.0, label = "Upper limit: 3.00 mmol/L"))
      }
      if (tn == "Systolic BP") {
        return(list(y = 140, label = "Upper limit: 140 mmHg"))
      }
      if (tn == "Diastolic BP") {
        return(list(y = 90, label = "Upper limit: 90 mmHg"))
      }
      if (tn == "HbA1c") {
        return(list(y = 48, label = "Treatment target: 48 mmol/mol"))
      }
    ...
    data <- dplyr::as_tibble(data) %>%
      ...
      dplyr::summarise(
        "{test_col}" := dplyr::first(.data[[test_col]]),
        "{unit_col}" := dplyr::first(.data[[unit_col]]),
        "{y_col}" := if (all(is.na(.data[[y_col]]))) NA_real_ else mean(.data[[y_col]], na.rm = TRUE),
        .groups = "drop"
      ) %>%
    ...
    # Keep the main label clean (the ref note will be added later in the combined subplot)
    y_label <- paste(test_col, "<br>(", data[[unit_col]][1], ")", sep = "")

    assay_name <- as.character(stats::na.omit(unique(data[[test_col]]))[1])
    ref <- get_reference_line(assay_name)
    ...
    plot <- ggplot(data, aes(x = !!sym(date_col), y = !!sym(y_col))) +
      ...
      scale_x_date(
        date_labels = "%d/%m/%Y",
        limits = c(as.Date("2020-01-01"), as.Date(Sys.Date()))
      ) +
      theme_minimal() +
    ...
    if (!is.null(ref)) {
      plot <- plot +
        geom_hline(yintercept = ref$y, linetype = "dashed", color = vis_colour, alpha = 0.6)
    }

    p <- ggplotly(plot, tooltip = "text") %>%
      layout(margin = list(l = 250))

    # Attach reference note so the combined subplot renderer can position it under the left label
    if (!is.null(ref)) {
      attr(p, "ref_note") <- ref$label
    } else {
      attr(p, "ref_note") <- NULL
    }
...
ui <- navbarPage(
  ...
  header = tags$head(
    tags$style(HTML("
      ...
      h1, h2, h3, h4 { color: #2F6FB5; }
      hr { color: #2F6FB5; border-color: #2F6FB5; }
      .navbar { background-color: #2F6FB5; border-color: #2F6FB5; }
      ...
      .btn-primary, .btn-download { background-color: #2F6FB5; border-color: #2F6FB5; color: #fff; }
      a { color: #2F6FB5; }
      .toc.float-toc { margin-bottom: 20px; }
      .plot-container { width: 100% !important; max-width: none !important; margin-left: 0 !important; margin-right: 0 !important; }
      .shiny-input-container p em { color: #666; }
...
fluidRow(
  column(
    ...
    p(tags$em("Please select your patient's name from the filter below."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
    div(
      class = "info-card",
      selectInput("patient_names", "Select Patient name:", choices = patient_names),
      uiOutput("preset_filter_ui")
    )
  ),
  column(
    width = 4,
    h3("Patient information"),
    p(tags$em("General patient information."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
  ...
  column(
    width = 4,
    h3("Clinical notes"),
    p(tags$em("Most recently available note: see 'Clinical Notes' tab for further information."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
...
fluidRow(
  column(
    width = 4,
    h3("Diagnoses"),
    p(tags$em("Date included relates to initial diagnosis."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
  ...
  column(
    width = 4,
    h3("Biomedical tests"),
    p(tags$em("Date included relates to latest test."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
  ...
  column(
    width = 4,
    h3("Prescriptions"),
    p(tags$em("Date included relates to initial prescription."), style = "margin-top:-10px; margin-bottom:6px; color:#666; font-size:11pt;"),
    hr(),
...
# row 3 (visalisations)
    ...
    fluidRow(
      column(
        ...
        actionButton(
          "toggle_visualisations",
          "Hide visualisations",
          class = "btn btn-default btn-sm"
        ),
        br(),
        div(class = "plot-outer", uiOutput("visualisations_ui")),
        br(),
        actionButton(
...
div(
  ...
  fluidRow(
    ...
    column(
      width = 3,
      uiOutput("notes_year_ui")
    ),
    column(
      width = 3,
      uiOutput("notes_month_ui")
    ),
    column(
      width = 2,
      checkboxInput(
        "notes_show_all",
        "Show all notes (ignore search filter)",
        value = FALSE
      )
    )
  )
...
# helper function to build dynamic y-axis labels for plots
...
# pop-out modal for clinical notes (click / keyboard)
observeEvent(input$note_click, {
  info <- input$note_click
  req(info$body)

  showModal(
    modalDialog(
      title = paste0(info$date, " • ", info$type, " • ", info$patient),
      tags$div(
...
# preset filter helpers
  ...
  preset_targets <- function(preset_id) {
    preset_id <- as.character(preset_id %||% "none")

    if (preset_id == "t2dm") {
      return(list(
        dx   = c("Type 2 diabetes"),
        lab  = c("HbA1c"),
        drug = c("Metformin")
      ))
    }
    if (preset_id == "htn") {
      return(list(
        dx   = c("Hypertension"),
        lab  = c("Systolic BP", "Diastolic BP"),
        drug = c("Amlodipine")
      ))
    }
    if (preset_id == "lipids") {
...
# defaulting to select everything available
    
    list(dx = NULL, lab = NULL, drug = NULL)
  }

  get_selected_for_patient <- function(all_choices, target_vec) {
    
    if (is.null(target_vec)) return(all_choices)

    out <- intersect(all_choices, target_vec)
...
# visualisations toggle (shown by default)
show_visualisations <- reactiveVal(TRUE)

observeEvent(input$toggle_visualisations, {
  show_visualisations(!show_visualisations())
})

output$visualisations_ui <- renderUI({
  if (!show_visualisations()) return(NULL)
  uiOutput("combined_plot_ui")
})
...
# clinical notes data (and month/year filters)

notes_data <- reactive({
  d <- Pt_Wide_reactive()
  nm <- if (nrow(d)) trimws(d$Patient_Name[1]) else "Selected patient"

# base notes (synthetic) for the selected patient
  
  notes <- make_fake_notes(nm)
  notes <- notes[order(notes$note_date, decreasing = TRUE), , drop = FALSE]

  notes$year  <- format(notes$note_date, "%Y")
  notes$month <- format(notes$note_date, "%B")
  notes$month_num <- as.integer(format(notes$note_date, "%m"))

  notes
})

output$notes_year_ui <- renderUI({
  notes <- notes_data()

  yrs <- sort(unique(notes$year), decreasing = TRUE)
  choices <- c("All years" = "all")
  choices <- c(choices, stats::setNames(yrs, yrs))

  current <- input$notes_year
  if (is.null(current) || !(current %in% unname(choices))) current <- "all"
...
output$notes_month_ui <- renderUI({
  notes <- notes_data()

  yr <- input$notes_year
  if (!is.null(yr) && yr != "all") {
    notes <- notes[notes$year == yr, , drop = FALSE]
  }
...
# ordering months chronologically based on month number
  
  month_df <- unique(notes[, c("month", "month_num")])
  month_df <- month_df[order(month_df$month_num), , drop = FALSE]
  mos <- month_df$month

  choices <- c("All months" = "all")
  choices <- c(choices, stats::setNames(mos, mos))

  current <- input$notes_month
  if (is.null(current) || !(current %in% unname(choices))) current <- "all"
...
# applying month/year filters
  
  yr <- input$notes_year
  mo <- input$notes_month

  if (!is.null(yr) && yr != "all") {
    notes <- notes[notes$year == yr, , drop = FALSE]
  }
  if (!is.null(mo) && mo != "all") {
    notes <- notes[notes$month == mo, , drop = FALSE]
  }
  ...
  if (nrow(notes) == 0) {
    return(tags$div(
      ...
      tags$div(class = "note-body", "Try a different search term, adjust the month/year filters, or tick 'Show all notes'.")
    ))
  }
  ...
  for (m in month_levels) {
    ...
    for (i in idx) {
      ...
      payload <- jsonlite::toJSON(
        list(
          date = format(notes$note_date[i], "%d %b %Y"),
          type = as.character(notes$note_type[i]),
          patient = nm,
          body = as.character(notes$note_body[i])
        ),
        auto_unbox = TRUE
      )
      ...
      out <- c(out, list(
        tags$div(
          class = paste("note-card clickable", if (is_latest) "pinned" else ""),
          role = "button",
          tabindex = "0",
          `aria-label` = paste0("Open note: ", meta_line),
          onclick = paste0("Shiny.setInputValue('note_click', ", payload, ", {priority:'event'});"),
          onkeydown = paste0(
            "if(event.key==='Enter' || event.key===' '){",
            "Shiny.setInputValue('note_click', ", payload, ", {priority:'event'});",
...
output$preset_filter_ui <- renderUI({
  
  dx <- dx_choices()

  choices <- c("None (show all)" = "none")

  if ("Type 2 diabetes" %in% dx) choices <- c(choices, "Type 2 diabetes" = "t2dm")
  if ("Hypertension" %in% dx) choices <- c(choices, "Hypertension" = "htn")
  if ("Lipid disorder" %in% dx) choices <- c(choices, "Lipid disorder" = "lipids")
  if ("Depression" %in% dx) choices <- c(choices, "Depression" = "depr")

  current <- input$preset_filter
  if (is.null(current) || !(current %in% unname(choices))) current <- "none"

  selectInput(
    inputId = "preset_filter",
    label   = "Select preset filter:",
    choices = choices,
    selected = current
  )
})
...
output$dx_filter_ui <- renderUI({
  ...
  preset <- preset_targets(input$preset_filter)
  selected_dx <- get_selected_for_patient(ch, preset$dx)
...
output$lab_filter_ui <- renderUI({
  ...
  preset <- preset_targets(input$preset_filter)
  selected_lab <- get_selected_for_patient(ch, preset$lab)
...
output$drug_filter_ui <- renderUI({
  ...
  preset <- preset_targets(input$preset_filter)
  selected_drug <- get_selected_for_patient(ch, preset$drug)
...
# resetting filters when the patient changes
...
# diagnoses (first recorded date)
    ...
    shiny::updateCheckboxGroupInput(
      session,
      "dx_filter",
      selected = get_selected_for_patient(
        if (nrow(dx_choices_df) > 0) dx_choices_df$Diagnosis else character(0),
        preset$dx
      )
    )
...
# biomedical tests (latest recorded date)
    ...
    shiny::updateCheckboxGroupInput(
      ...
      "lab_filter",
      selected = get_selected_for_patient(
        if (nrow(lab_choices_df) > 0) lab_choices_df$Test else character(0),
        preset$lab
      )
    )
...
# prescriptions (first recorded date)
    ...
    shiny::updateCheckboxGroupInput(
      ...
      "drug_filter",
      selected = get_selected_for_patient(
        if (nrow(rx_choices_df) > 0) rx_choices_df$Drug else character(0),
        preset$drug
      )
    )
  }, ignoreInit = FALSE)

# applying preset selections immediately when the preset dropdown changes
  
  observeEvent(input$preset_filter, {
    d <- Pt_Wide_reactive()
    ...
    dx_all <- dx_choices()
    lab_all <- lab_choices()
    drug_all <- drug_choices()

    shiny::updateCheckboxGroupInput(session, "dx_filter",
                                   selected = get_selected_for_patient(dx_all, preset$dx))
    shiny::updateCheckboxGroupInput(session, "lab_filter",
                                   selected = get_selected_for_patient(lab_all, preset$lab))
    shiny::updateCheckboxGroupInput(session, "drug_filter",
                                   selected = get_selected_for_patient(drug_all, preset$drug))
  }, ignoreInit = TRUE)
...
combined_height <- reactive({
  ...
  plots <- list(
    ...
    create_time_series_plot(l1_d, "Contact_Event_Date", "Biomedical_Value_1", "Biomedical_Test_1", "Biomedical_1_Units", "#2F6FB5"),
    create_time_series_plot(l2_d, "Contact_Event_Date", "Biomedical_Value_2", "Biomedical_Test_2", "Biomedical_2_Units", "#2F6FB5"),
...
per_panel_height <- 130
base_margin_px   <- 60
raw_height <- max(200, n_panels * per_panel_height + base_margin_px)

# capping max height of containers

max_height <- 760
min(raw_height, max_height)
})
...
output$combined_plot <- renderPlotly({
  ...
  combined_plot <- plotly::subplot(
    ...
    layout(
      annotations = {
        base_ann <- lapply(seq_along(valid_labels), function(i) {
          list(
            x = -0.10,
        ...
        ref_ann <- lapply(seq_along(valid_plots), function(i) {
          note <- attr(valid_plots[[i]], "ref_note")
          if (is.null(note) || is.na(note) || !nzchar(note)) return(NULL)

          y_main <- 1 - (i - 1) * (1 / n_panels) - 0.11

          list(
            x = -0.10,
            y = y_main - 0.045,              # sits just underneath the main label
            text = paste0("<i>", note, "</i>"),
            showarrow = FALSE,
            xref = "paper",
            yref = "paper",
            xanchor = "center",
            yanchor = "middle",
            textangle = 0,
            font = list(size = 12, color = "#2F6FB5")
          )
        })

        c(base_ann, Filter(Negate(is.null), ref_ann))
      },
...
generate_fake_nhs_number <- function(seed_int) {

  make_one <- function() {
    d <- sample(0:9, size = 9, replace = TRUE)
  ...
  out <- NULL
  while (is.null(out)) out <- make_one()

  paste0(substr(out, 1, 3), " ", substr(out, 4, 6), " ", substr(out, 7, 10))
}
...
output$patient_report <- renderUI({
  ...
  nhs_number <- generate_fake_nhs_number(seed)
...
# fictional placeholder metrics
  ...
  height_m_internal <- round(runif(1, 1.50, 1.90), 2)
  bmi <- round(weight_kg / (height_m_internal^2), 1)
...
# residential / carer status
  
  residential_options <- c("Lives alone", "Lives with family", "Assisted living")
  residential_status <- sample(residential_options, 1, prob = c(0.40, 0.45, 0.15))

  if (residential_status == "Assisted living") {
    carer_options <- c("Full time care", "Has part-time carer (non-family)")
    carer_status <- sample(carer_options, 1, prob = c(0.70, 0.30))
  } else if (residential_status == "Lives alone") {
    carer_options <- c("No carer", "Has part-time carer (family)", "Has part-time carer (non-family)", "Full time care")
    carer_status <- sample(carer_options, 1, prob = c(0.55, 0.20, 0.20, 0.05))
  } else { 
    carer_options <- c("No carer", "Has part-time carer (family)", "Has part-time carer (non-family)", "Full time care")
    carer_status <- sample(carer_options, 1, prob = c(0.45, 0.40, 0.10, 0.05))
  }

  tagList(
    tags$p(
      tags$strong("NHS Number: "), nhs_number, tags$br(),
      ...
      tags$strong("Allergies: "), allergies, tags$br(),
      tags$strong("Residential status: "), residential_status, tags$br(),
      tags$strong("Carer status: "), carer_status
    )
  )
})
