...
ui <- navbarPage(
  ...
  header = tags$head(
    tags$style(HTML("
      ...
      .plot-container { width: 100% !important; max-width: 1700px !important; margin-left: auto !important; margin-right: auto !important; }
      .shiny-input-container p em { color: #666; }
...
# summary tables
  ...
  dx_table_data <- reactive({
    ...
    dx <- d %>%
      ...
      mutate(Contact_Event_Date = as.Date(Contact_Event_Date)) %>%
      distinct(Contact_Event_Date, Diagnosis) %>%
...
# sisualisations toggle (shown by default)
...
# preset dropdown UI (dynamic per selected patient)
