...
create_time_series_plot <- function(data, date_col, y_col, test_col, unit_col, vis_colour) {
  tryCatch({
    ...
    # midpoint date for centered annotation
    
    x_mid <- as.Date(mean(as.numeric(range(data[[date_col]], na.rm = TRUE))), origin = "1970-01-01")

    y_range_vals <- range(data[[y_col]], na.rm = TRUE)
    y_rng <- diff(y_range_vals)
    y_offset <- if (is.finite(y_rng) && y_rng > 0) max(y_rng * 0.07, 0.25) else 0.25
    ...
    if (!is.null(ref)) {
      ...
      y_label_pos <- ref$y - y_offset

      y_min_safe <- y_range_vals[1] + if (is.finite(y_rng) && y_rng > 0) (y_rng * 0.02) else 0
      if (is.finite(y_label_pos) && is.finite(y_min_safe)) {
        y_label_pos <- max(y_label_pos, y_min_safe)
      }
