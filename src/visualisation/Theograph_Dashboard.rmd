...
# Reactive expression to filter data by selected patient
  ...
  dx_table_data <- reactive({
    ...
    if (!is.null(input$dx_filter) && length(input$dx_filter) > 0) {
  ...
  rx_table_data <- reactive({
    ...
    if (!is.null(input$drug_filter) && length(input$drug_filter) > 0) {
  ...
  lab_table_data <- reactive({
    ...
    if (!is.null(input$lab_filter) && length(input$lab_filter) > 0) {
...
output$dx_filter_ui <- renderUI({
    ...
    labels <- lapply(seq_along(ch), function(i) {
      dt <- dates[[i]]
      dt_txt <- if (!is.na(dt)) format(dt, "%d %b %Y") else ""
      htmltools::HTML(paste0(htmltools::htmlEscape(ch[[i]]), "&nbsp;&nbsp;<em>", dt_txt, "</em>"))
    })

    checkboxGroupHTML("dx_filter", "Diagnoses", ch, labels, selected = ch)
  })

  output$lab_filter_ui <- renderUI({
    d <- Pt_Wide_reactive()

    # Building tabular view of patient data
    
    lab_table_data <- d %>%
      dplyr::select(Contact_Event_Date, Biomedical_Test_1, Biomedical_Test_2, Biomedical_Test_1_Value, Biomedical_Test_2_Value, Biomedical_Test_1_Unit, Biomedical_Test_2_Unit) %>%
      tidyr::pivot_longer(
      ...
      dplyr::mutate(
        Value = dplyr::if_else(Test_Number == "Biomedical_Test_1", Biomedical_Test_1_Value, Biomedical_Test_2_Value),
        Unit  = dplyr::if_else(Test_Number == "Biomedical_Test_1", Biomedical_Test_1_Unit,  Biomedical_Test_2_Unit)
      ) %>%
      dplyr::filter(!is.na(Test), Test != "")

    choice_vals <- sort(unique(lab_table_data$Test))
    if (length(choice_vals) == 0) {
      return(NULL)
    }

    latest_dates <- lab_table_data %>%
      dplyr::group_by(Test) %>%
      dplyr::summarise(
        latest_date = if (all(is.na(Contact_Event_Date))) as.Date(NA) else max(Contact_Event_Date, na.rm = TRUE),
        .groups = "drop"
      )

    labels <- lapply(choice_vals, function(test) {
      dt <- latest_dates$latest_date[match(test, latest_dates$Test)]
      dt_txt <- if (!is.na(dt)) format(dt, "%d %b %Y") else ""
      ...
      if (dt_txt == "") {
        ...
        tagList(
          test,
          span(style = "margin-left: 10px; font-style: italic;", dt_txt)
        )
      }
    })

    checkboxGroupHTML("lab_filter", "Biomedical tests / assays", choice_vals, labels, selected = choice_vals)
  })
  ...
  output$drug_filter_ui <- renderUI({
    ...
    checkboxGroupHTML("drug_filter", "Drugs", ch, labels, selected = ch)
  })
